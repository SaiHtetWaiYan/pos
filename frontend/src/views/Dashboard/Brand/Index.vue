  ...
  <div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        ...
        <p class="mt-2 text-sm text-gray-700">A list of all the brands in your account created name and show.</p>
      </div>
        ...
        <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
          <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
            <table class="min-w-full divide-y divide-gray-300">
              ...
              <tr v-for="(brand, brandIdx) in brands.data" :key="brand.id" :class="brandIdx % 2 === 0 ? undefined : 'bg-gray-50'">
                <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-500 sm:pl-6">{{ (brandIdx+ 1 + brands.current_page * brands.per_page) - brands.per_page   }}</td>
                <td class="whitespace-nowrap px-4 py-4 text-sm text-gray-500">{{ brand.name }}</td>
                <td class="whitespace-nowrap py-4 text-sm text-gray-500">
                  <span v-if="brand.is_show === 1">Yes</span>
                  <span v-else>No</span>
                </td>
              ...
              <tr v-if="brands.total === 0">
                <td class="px-6 py-4 whitespace-nowrap border-b border-gray-200 text-center" colspan="4">No brands found.</td>
              </tr>
              </tbody>
            </table>
          </div>
...
import Create from "@/views/Dashboard/Brand/Create.vue"
import Edit from "@/views/Dashboard/Brand/Edit.vue"
...
export default {
  ...
  methods:{
    async fetchdata(){
      const response = await axiosInstance.post('/api/brand?page=' + this.page ,{
        search : this.search,
        perpage : this.per_page,
        trashed : this.trashed
      })
      this.brands = response.data.brands
      this.pageCount = response.data.brands.last_page
    },
    getData(value){
      this.fetchdata()
      this.msg = value
