  ...
  <div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        ...
        <p class="mt-2 text-sm text-gray-700">A list of all the categories in your account created name and show.</p>
      </div>
          ...
          <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
            <table class="min-w-full divide-y divide-gray-300">
              ...
              <tr>
                ...
                <th scope="col" class="py-3.5 text-left text-sm font-semibold text-gray-900">Is_Show</th>
              ...
              <tr v-for="(category, categoryIdx) in categories.data" :key="category.id" :class="categoryIdx % 2 === 0 ? undefined : 'bg-gray-50'">
                <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-500 sm:pl-6">{{ (categoryIdx+ 1 + categories.current_page * categories.per_page) - categories.per_page   }}</td>
                <td class="whitespace-nowrap px-4 py-4 text-sm text-gray-500">{{ category.name }}</td>
                <td class="whitespace-nowrap py-4 text-sm text-gray-500">
                  <span v-if="category.is_show === 1">Yes</span>
                  <span v-else>No</span>
                </td>
              ...
              <tr v-if="categories.total === 0">
                <td class="px-6 py-4 whitespace-nowrap border-b border-gray-200 text-center" colspan="4">No categories found.</td>
              </tr>
              </tbody>
            </table>
          </div>
...
import Create from "@/views/Dashboard/Category/Create.vue"
import Edit from "@/views/Dashboard/Category/Edit.vue"
...
export default {
  ...
  methods:{
    async fetchdata(){
      const response = await axiosInstance.post('/api/category?page=' + this.page ,{
        search : this.search,
        perpage : this.per_page,
        trashed : this.trashed
      })
      this.categories = response.data.categories
      this.pageCount = response.data.categories.last_page
    },
    getData(value){
      this.fetchdata()
      this.msg = value
